...
app.options('/auth/change-project', cors({
  origin: enviroment.ALLOWED_ORIGINS,
  methods: 'POST',
}))
app.post('/auth/change-project', cors({
  origin: enviroment.ALLOWED_ORIGINS,
  methods: 'POST',
}), async (req, res, next) => {
  const { body } = req;
  ...
  try {
    ...
    if (data.token.project !== undefined && Array.isArray(data.token.roles)) {
      let allData = { tokenMetadata: { tokenId: projectScopedToken2.response.headers._headers['x-subject-token'][0], scope: 'projectScope' }, ...projectScopedToken2.data }

      // console.log("allProjects links = ", allProjects)
      res.status(projectScopedToken2.response.status).json({ tokenMetadata: { tokenId: projectScopedToken2.response.headers._headers['x-subject-token'][0], scope: 'projectScope' }, ...projectScopedToken2.data })
    } else {
      res.status(projectScopedToken2.response.status).json({ tokenMetadata: { tokenId: projectScopedToken2.response.headers._headers['x-subject-token'][0], scope: 'unscoped' }, ...projectScopedToken2.data })
    }
...
app.post('/auth/login', cors({
  ...
  try {
      ...
      if (domains.length !== 0) {
        ...
        try {
          ...
          if (data.token.project !== undefined && Array.isArray(data.token.roles)) {
            ...
            // console.log("allProjects links = ", allProjects)
            res.status(result.response.status).json({ tokenMetadata: { tokenId: projectScopedToken.response.headers._headers['x-subject-token'][0], scope: 'projectScope' }, ...projectScopedToken.data, 'allProjects': allProjects.data.projects, 'unscopedToken': unscopedToken[0] })
          } else {
            res.status(result.response.status).json({ tokenMetadata: { tokenId: projectScopedToken.response.headers._headers['x-subject-token'][0], scope: 'unscoped' }, ...projectScopedToken.data })
          }
    ...
    console.log(e);
    res.status(e.data.error.code).json({ body: { message: "Invalid Credentials" } })
  }
});


// Get Authoenticate token
// app.options('/auth/verify-token', cors({
//   origin: enviroment.ALLOWED_ORIGINS,
//   methods: 'GET',
